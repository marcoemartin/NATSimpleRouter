...
uint32_t generate_masked(uint32_t ip_address, uint32_t mask) {
    return (ip_address & mask);
}

/**
 * Fetch route matching given ip_address using longest prefix match.
 * Returns NULL if not match is found.
 */
struct sr_rt* get_route(struct sr_rt* routing_table, uint32_t ip_address) {
    ...
    while(current != NULL) {
        ...
        uint32_t ip_address_masked = generate_masked(ip_address, current->mask.s_addr);
        uint32_t dest_masked = generate_masked(current->dest.s_addr, current->mask.s_addr);

        if(ip_address_masked == dest_masked) {
            if(found == NULL ||
                    /*
                        choose candidate with 'largest' mask.
                        mask is assumed to be properly formatted such that it
                        satisfies CIDR notation.
                    */
                    (current->mask.s_addr > found->mask.s_addr)) {
                found = current;
            }
        }
